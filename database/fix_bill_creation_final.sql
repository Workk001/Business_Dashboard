-- Final fix for bill creation issues
-- This script addresses the "bill_number" field error and ensures proper bill creation

-- 1. First, let's check if there are any triggers or functions that might be causing the bill_number issue
-- Drop any existing triggers that might reference bill_number
DROP TRIGGER IF EXISTS set_bill_number_trigger ON bills;
DROP TRIGGER IF EXISTS generate_bill_number_trigger ON bills;

-- 2. Drop any functions that might be setting bill_number
DROP FUNCTION IF EXISTS generate_bill_number();
DROP FUNCTION IF EXISTS set_bill_number();
...
-- 5. Create a function to generate invoice numbers (optional)
